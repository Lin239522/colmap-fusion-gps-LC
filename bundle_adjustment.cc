...
void BundleAdjuster::AddImageToProblem(const image_t image_id,
  ...
  // OS 1 从重建对象 reconstruction 中获取要添加到BA的图像对象 image 和相机对象 camera。
  Image& image = reconstruction->Image(image_id);
  Camera& camera = reconstruction->Camera(image.CameraId());

  // OS 2 对图像的四元数进行归一化操作，确保其为单位四元数，以符合Ceres Solver的要求 CostFunction assumes unit quaternions.
  // CostFunction assumes unit quaternions.
  image.NormalizeQvec();

  // OS 3 提取图像的四元数、平移向量以及相机的内参，这些数据将用于构建残差项
  double* qvec_data = image.Qvec().data();
  double* tvec_data = image.Tvec().data();
  double* camera_params_data = camera.ParamsData();

  // OS 4 根据图像是否具有常数位姿（即是否被设置为不优化的图像），选择合适的 cost_function 来构建残差项
